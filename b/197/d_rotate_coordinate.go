...
func main() {
	...
	cx := (float64(xz) + float64(xn2)) / 2
	cy := (float64(yz) + float64(yn2)) / 2

	ax, ay := rotate(360 / float64(n) * math.Pi / 180, (float64(xz) - cx), (float64(yz) - cy))

	iou.Pl(ax + cx, ay + cy)
}

func rotate(rad, x, y float64) (float64, float64) {
