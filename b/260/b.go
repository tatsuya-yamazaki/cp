...
func main() {
	...
	for i:=0; i<n; i++ {
		sti := st{i+1, a[i], b[i], a[i]+b[i], false}
		s[i] = sti
	}

	sort.SliceStable(s, func(i, j int) bool {return s[i].a > s[j].a })
	f(s, x)
	sort.SliceStable(s, func(i, j int) bool {return s[i].i < s[j].i })
	sort.SliceStable(s, func(i, j int) bool {return s[i].b > s[j].b })
	f(s, y)
	sort.SliceStable(s, func(i, j int) bool {return s[i].i < s[j].i })
